    ...
    if (props.pick.teamPicked === props.tileTeam.teamID) {
        if (props.game.gameFinished) {
            ...
            if (
                ...
                cellStyling = "bg-radial from-[#08e63a] h-full m-auto rounded-4xl to-[#1E1E1E] w-full";
            } else {
                // Game Finished and Pick is Incorrect
                cellStyling = "bg-radial from-[#bb4343] h-full m-auto rounded-4xl to-[#1E1E1E] w-full";
            }
        } else {
            cellStyling = `bg-radial from-[20%] from-[#FAFAFA] border-5 h-full rounded-4xl w-full`;
        }
    }
